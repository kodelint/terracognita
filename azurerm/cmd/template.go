...
const (
	...
	func (ar *AzureReader) List{{ .Name }}(ctx context.Context) ([]{{ .API }}.{{ .Resource }}, error) {
		client := {{ .API }}.New{{ .Resource }}sClient(ar.config.SubscriptionID)
		client.Authorizer = ar.authorizer

		output, err := client.{{ .ListFunction }}(ctx{{ if .Location }}, ar.GetLocation(){{ end }}{{ if .ResourceGroup }}, ar.GetResourceGroupName(){{ end }})
		if err != nil {
